...
this.connect('scroll-event', (_, event) => {
    ...
    index = Math.clamp(index, 0, wsManager.n_workspaces - 1);
    wsManager.get_workspace_by_index(index).activate(global.get_current_time());

    // popup 表示
    if (!Main.wm._workspaceSwitcherPopup) {
        //const { WorkspaceSwitcherPopup } =
            //import { WorkspaceSwitcherPopup }from 'resource:///org/gnome/shell/ui/workspaceSwitcherPopup.js' //;imports.ui.windowManager; // (= resource:///org/gnome/shell/ui/windowManager.js)
        if (this._workspaceSwitcherPopup) {
            this._workspaceSwitcherPopup.destroy();
            this._workspaceSwitcherPopup = null;
        }
        this._workspaceSwitcherPopup = new WorkspaceSwitcherPopup.WorkspaceSwitcherPopup();
        this._workspaceSwitcherPopup.display(index);
    }
    ...
    handleDragOver(source, _actor, _x, _y, _time) {
       ...
       const result = Main.wm.handleWorkspaceScroll(event);

        // === workspace-switcher popup を表示 ===
        if (event.type() === Clutter.EventType.SCROLL) {
            ...
            // アクティブワークスペースの番号を渡して表示
            this._workspaceSwitcherPopup.display(wm.get_active_workspace().index());
        }
