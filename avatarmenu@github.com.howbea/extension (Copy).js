...
let itemal = new PopupMenu.PopupSwitchMenuItem(
    _('Automatic Screen Lock'),
    this._syssettings.get_boolean('lock-enabled')
);

// トグル
itemal.connect('toggled', (item, state) => {
    this._syssettings.set_boolean('lock-enabled', state);
    return Clutter.EVENT_STOP;
});

// 設定変更を反映
this._syssettings.connect('changed::lock-enabled', () => {
    itemal.setToggleState(this._syssettings.get_boolean('lock-enabled'));
});
...
export default class IndicatorExampleExtension extends Extension {
    ...
    enable() {
        ...
        Main.panel.addToStatusArea(this.uuid, this._indicator, 'right'); //8, 'center');
    }
