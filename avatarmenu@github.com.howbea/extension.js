...
class ActivitiesMenuButton extends PanelMenu.Button {
    _init() {
        ...
        let bin = new St.Bin({name: 'avatarMenu'});
        this.add_child(bin);
        ...
        this.user_label = new userWidget.UserWidgetLabel(user);  
        this._label = new St.Label({
            //text: this.user_label.text, //_('debian'),
            y_align: Clutter.ActorAlign.CENTER,
            style_class: 'avatar-label',
        });
        //this._label.text = this.user_label;
        //this._container.add_child(this.user_label); //this._label);
        
        const icon = new St.Icon({
            icon_name: 'avatar-default-symbolic', //'system-shutdown-symbolic', //'avatar-default-symbolic',
    ...
    _createSubMenu() {
        ...
        //this.box.add_child(this._userWidget);
    ...
    menu_build() {
        ...
        let itemusers = new PopupMenu.PopupMenuItem(_('User Settings')); //'org.gnome.Settings-users-symbolic');
        itemusers.connect('activate', () => {
        Shell.AppSystem.get_default().lookup_app('gnome-users-panel.desktop').activate();
        });
        
        let itemprivacy = new PopupMenu.PopupImageMenuItem(_('Privacy & Security'), 'org.gnome.Settings-privacy-symbolic');
        itemprivacy.connect('activate', () => {
        Shell.AppSystem.get_default().lookup_app('gnome-privacy-panel.desktop').activate();
        });
        
        let itemonline = new PopupMenu.PopupImageMenuItem(_('Online Accounts'), 'org.gnome.Settings-online-accounts-symbolic');
        itemonline.connect('activate', () => {
        Shell.AppSystem.get_default().lookup_app('gnome-online-accounts-panel.desktop').activate();
        });
        
        let itemmultitasking = new PopupMenu.PopupImageMenuItem(_('Multitasking'), 'org.gnome.Settings-multitasking-symbolic');
        itemmultitasking.connect('activate', () => {
        Shell.AppSystem.get_default().lookup_app('gnome-multitasking-panel.desktop').activate();
        });
        
        let itemappearance = new PopupMenu.PopupImageMenuItem(_('Appearance'), 'org.gnome.Settings-appearance-symbolic');
        itemappearance.connect('activate', () => {
        Shell.AppSystem.get_default().lookup_app('gnome-appearance-panel.desktop').activate();
        });
        
        let itemwellbeing = new PopupMenu.PopupImageMenuItem(_('Wellbeing'), 'org.gnome.Settings-wellbeing-symbolic');
        itemwellbeing.connect('activate', () => {
        Shell.AppSystem.get_default().lookup_app('gnome-wellbeing-panel.desktop').activate();
        });
        ...
        let itemscreenshot = new PopupMenu.PopupImageMenuItem(_('Take Screenshot'), 'screenshooter-symbolic');
...
this._syssettings = new Gio.Settings({ schema_id: 'org.gnome.desktop.screensaver' })

let itemal = new PopupMenu.PopupMenuItem(
    _('Automatic Screen Lock'), {style_class: 'qqq'}
);

if (this._syssettings.get_boolean('lock-enabled')) {
    itemal.label.text = 'Automatic Screen Lock  :   ON';
    //itemal._icon.icon_name = 'changes-prevent-symbolic';
} else {
    itemal.label.text = 'Automatic Screen Lock  :   OFF';
    //itemal._icon.icon_name = 'changes-allow-symbolic';
}

// トグル
itemal.connect('activate', () => {
    this._syssettings.set_boolean('lock-enabled', !this._syssettings.get_boolean('lock-enabled'));
});

// 設定変更を反映
this._syssettings.connect('changed::lock-enabled', () => {
    if (this._syssettings.get_boolean('lock-enabled')) {
    itemal.label.text = 'Automatic Screen Lock  :   ON';
    //itemal._icon.icon_name = 'changes-prevent-symbolic';
} else {
...
//let itemal = new PopupMenu.PopupImageMenuItem(_('Auto Lock'), 'changes-prevent-symbolic');
                let itemname = new PopupMenu.PopupMenuItem(_('Show name on panel'));
        
        //const enabledn = this._syssettings.get_boolean('panel-show-name');
//if (enabledn) {
  //  itemname.setOrnament(PopupMenu.Ornament.NONE);
//} else {
    itemname.setOrnament(PopupMenu.Ornament.CHECK);
//}
        
        itemname.connect('activate', () => {
    //const enabledn = this._syssettings.get_boolean('panel-show-name');
    //this._settings.set_boolean('panel-show-name', !enabled);
...
//}
    ...
    //this._settings.connect('changed::panel-show-name', () => {
    //const enabled = this._settings.get_boolean('panel-show-name');
...
//});
        ...
        this.menu.addMenuItem(itemwellbeing);
        ...
        //this.menu.addMenuItem(this._loginScreenItem);
        //this.menu.addMenuItem(this._sessionSubMenu);
        ...
        //this.menu.addMenuItem(itemname);
        this.menu.box.add_child(spaceitem);
    }
...
export default class IndicatorExampleExtension extends Extension {
    ...
    enable() {
        ...
        Main.panel.addToStatusArea(this.uuid, this._indicator, 5, 'right');
    }
